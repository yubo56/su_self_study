...
\begin{align}
    H &= \frac{\sum_j p_j^2 + \omega_j^2q_j^2}{2} = \sum_j I_j
\end{align}
...
$\dot{z}_j &= iz_j = i\pd{H}{z_j}$. Thus, we see that the Hamiltonian must be
$H = \sum_j\frac{z_j^2}{2}$\footnote{We have made a choice of convention in
...
real-variable Hamiltonian}, and it produces dynamics that can be expressed in
terms of the $\vec{z}$ vector as $\dot{\vec{z}} = i\vec{z}$\footnote{This seems
eerily similar to the quantum mechanical analog, where $p = -i\hbar \pd{}{x}$.
Probably not coincidence but a simple property of the symplectic phase space our
dynamics exist in.}.
