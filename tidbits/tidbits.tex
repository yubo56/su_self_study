...
We know that with difference equations, solutions take on form $\alpha^k$ with
$\alpha$ an undetermined variable. Making this ansatz gives us
\begin{align}
    \alpha^{i+1} - 2\alpha^i + \alpha^{i-1} &= C_{ij}
\end{align}

One possible choice for $C_{ij}$ now arises: $\lambda V_{i,j}$. This gives us
\begin{align}
    \alpha^2 - (2+\lambda)\alpha + 1 &= 0
\end{align}

There are two kinds of solutions to this, where both are real and have the same
sign, or where both are complex conjugates.
\end{document}
