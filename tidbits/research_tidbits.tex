...
We can perhaps see a path forward.
...
\begin{align}
    ...
    \bm{s}_{10}
        &= -\frac{\alpha}{\bar{g}}
            \bm{l}_{\perp} - \p{\uv{s}_{00} \cdot \bm{l}_{\perp}}
        \uv{s}_{00}.
\end{align}
...
\bar{g})$), whiel its rotation has frequency $\bar{g}$ (Eq.~\ref{eq:s10_prec}).
Consistency among all of these frequencies demands $\bar{g} = (g_1 + g_2) / 2$.

\subsection{Co-precessing, co-nutating approach}

\textbf{For posteritty, I include the erroneous approach I started with\dots}
