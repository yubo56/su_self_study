...
We aren't quite ready to apply Brouwer's Theorem, since this domain is not
closed: it's a little sus for the domain to include $e=1$.
However, we can invoke a second constraint: depending on the system's AMD, if it
is sufficiently low, then there are no unbound orbits.
Then, the entire space of AM-conserving solutions must form a closed (compact)
subset of $D$.

I lied, the correct theorem to apply here is the hairy ball theorem.
Note that AMD conservation restricts us to a 2D subspace of $D$, which is
isomorphic to $S^2$ but \emph{not} any unit disk!
Does BFPT also apply to $S^2$?
Probably, but I'm not smart enough to prove it.

But with the hairy ball theorem, our problem is done.
The secular evolution will map every point in $D_t$ to its point at $D_{t +
\delta t}$.
We have a map, or a vector field, on a surface homeomorphic to $S^2$ (probably,
though may need to check).
By the HBT, there must be a point where the vector field vanishes.
This is the fixed point of the secular evolution.

In the inclination case, this is an obvious result: it's just the total AM of
the system.
In the eccentricity case, I'm sure it's well-known, but it's new to me.
Of course, in the Laplace-Lagrange case, if $m_2 \gg m_1$, this is just the
forced eccentricity for the inner planet, or the COM eccentricity of the
two-planet system (Hadden must have such a paper?).
In the multipolar expansion case, it's the center of libration of the CHEM
Hamiltonian.

But the key point is that we don't need to assume either LL or hierarchical
systems to show that there must be a ``center of mass eccentricity'' in the
system.
So long as the system's evolution is bounded by AMD constraints and secular
interactions, the Brouwer Fixed-Point Theorem is enough to guarantee such a
solution.

Now, proving the BFPT or HBT is true is far beyond me.
I know some of the famous topology results are on 3blue1brown, but I don't think
this one is there?
The proof I saw of BFPT (that spawned this writeup) uses some category
theory\dots
I think the rough idea is that: any map from D (call it the disk) to itself maps
a point $p_1$ to $p_1'$.
If these two are not equal, we can draw the line $p_1p_1'$ and mark the
intersection with the boundary of the disk, and thus every map from $D$ to
itself corresponds to a map from $D$ onto its boundary.
But it's apparently possible to show that something about the categories of
these two maps is incompatible\dots
Sounds nifty, but I'm never gonna understand it.

But also, note that the HBT only guarantees a single fixed point.
Thus, this is only enough to show that, for a given set of precessional
dynamics, there is one fixed point.
This contrasts with our intuition from the inclination case, where both $\pm$
the total AM correspond to fixed points.
A quick Wikipedia read shows that only pretty convoluted maps will give exactly
one ``pole'' of the vector field (where it vanishes), so in the astrophysical
setting, we're probably fine.
