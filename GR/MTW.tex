...
\begin{description}
    ...
    \item[Hyperbolic Motion] If we require $\mathbf{a} = \rd{\mathbf{u}}{\tau}$ to
        satisfy $0 = \mathbf{a} \cdot \mathbf{u}$ (necessitated if we require
        $\mathbf{u}^2 = -1$ to not instantaneously change), then we can show
...
\begin{description}
    \item[Self] \emph{Derive hyperbolic motion.}
        ...
        The assumptions are: $\mathbf{a}^\mu\mathbf{a}_\mu = g^2$ constant
        magnitude, $\mathbf{u}^\mu\mathbf{u}_\mu = -1$ unaccelerated, and
        $\mathbf{a} \cdot \mathbf{u} = 0$ at all times. Go to the instaneous
        rest frame of the motion at any time, then since $\mathbf{u} = (1, 0, 0,
        0)$, we note $a_0 = 0$. Choose $\mathbf{a} \propto \mathbf{e}_1$ for
        ...
        \begin{align*}
            ...
            \rd{u_0}{t} &= gu_1
        \end{align*}
        which exhibits solutions $u_1(t) = \sinh(gt), u_0(t) = \cosh(gt)$,
        which exhibits the desired property $x_0^2 - x_1^2 = g^{-2}$.

    \item[6.1] \emph{Compute the proper time for an observer to travel distance
        $L$ at acceleration $g$ for half, then $-g$ for the other half.}

    \item[6.6] \emph{Show that in the LLT coordinate system $\xi^\alpha$, the
        proper time measured at coordinate $\xi^1$ is given $\rd{\tau}{\xi^0} =
        1 + g\xi^1$.}
